...
/**
 * Demand-driven activation trigger (D17 / RF): while the recent request-rejection rate
 * ({@link GuidanceState#recentRejectionRate()}) exceeds {@code rejectionRateThreshold}, it targets the full activation
 * capacity {@code Σκ} — i.e. "under demand pressure, put every vehicle the operator pool can supervise into service".
 * Below the threshold it proposes nothing (target 0), so the responsiveness buffer and the regulatory floor govern the
...
public final class RejectionRateActivation implements ActivationTrigger {
	...
	public int desiredActive(GuidanceState state, double now) {
		// strictly ABOVE the threshold → go to the ceiling; the reconciler clamps to the actual Σκ anyway, so naming the
